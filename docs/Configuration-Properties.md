...
| `powerauth.service.crypto.temporaryActivationBlock.enabled`              | `false`     | Whether the temporary activation block feature is enabled. Applies to activations using cryptography protocol v4. When enabled, such activations that reach the maximum number of failed attempts are blocked for a limited time and automatically returned to the ACTIVE state. Activations using older protocol versions are blocked permanently regardless of this flag. |
| `powerauth.service.crypto.temporaryActivationBlock.periodInMilliseconds` | `300000`    | Time period (in milliseconds) for the first temporary activation block. Default 5 minutes.                                                                                                                                                                                                                                                                                  |
| `powerauth.service.crypto.temporaryActivationBlock.multiplier`           | `2`         | Multiplier applied to the temporary block period for consecutive blocks. The n-th consecutive block lasts `periodInMilliseconds * multiplier^(n-1)` ms.                                                                                                                                                                                                                     |
...
| `powerauth.server.db.master.encryption.aead-kmac.key`                    | `_empty_`   | 32-byte Base64-encoded master encryption key required for the `AEAD_KMAC` algorithm.                                                                                                                                                                                                                                                                                        |
...
| `powerauth.service.rest.date.legacyFormatEnabled`                        | `false`     | When `true`, serializes REST API and callback timestamps using the legacy numeric UTC offset (e.g. `2026-08-05T10:47:09.449+00:00`) instead of the default `Z` designator (e.g. `2026-08-05T10:47:09.449Z`). Both are valid ISO-8601; enable only for legacy clients that cannot parse `Z`. Serialization only; deserialization always accepts both forms.                    |
...
| `powerauth.service.scheduled.job.expireTemporaryActivationBlocks`           | `5000`          | Time delay in milliseconds between two consecutive tasks that expire temporary activation blocks.                                           |
