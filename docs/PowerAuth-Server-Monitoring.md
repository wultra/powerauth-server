...
- Overall health  
  `GET /powerauth-java-server/actuator/health`
- Liveness probe  
  `GET /powerauth-java-server/actuator/health/liveness`
- Readiness probe  
  `GET /powerauth-java-server/actuator/health/readiness`
...
The management (Actuator) endpoints can be exposed on a dedicated port to separate them from the standard application traffic. When a separate port is configured, Actuator endpoints will be available directly under the /actuator context (i.e., without /powerauth-java-server).
...
PowerAuth Server can be monitored via other tools as well. Example configuration to enable monitoring via OpenTelemetry:
...
OTEL_SERVICE_NAME=powerauth-server
```
...
This section describes what should be monitored when operating the PowerAuth Server in production and how it maps to the metrics **exposed by the server itself** (via Spring Boot / Micrometer / Prometheus).
...
PowerAuth Server uses a JDBC connection pool **HikariCP**. Monitoring the pool is critical to detect database saturation and connection issues.
...
- `hikaricp_connections{pool="HikariPool-PowerAuth-Server", state="active"}`
- `hikaricp_connections{pool="HikariPool-PowerAuth-Server", state="idle"}`
- `hikaricp_connections{pool="HikariPool-PowerAuth-Server", state="pending"}`
- `hikaricp_connections{pool="HikariPool-PowerAuth-Server", state="max"}`
...
- `hikaricp_connections_acquire_seconds_sum{pool="HikariPool-PowerAuth-Server"}`
- `hikaricp_connections_acquire_seconds_count{pool="HikariPool-PowerAuth-Server"}`
- `hikaricp_connections_acquire_seconds_max{pool="HikariPool-PowerAuth-Server"}`
...
- `application` / `service` – `powerauth-server`
...
- Status returned by:
    - `/powerauth-java-server/actuator/health`
    - `/powerauth-java-server/actuator/health/liveness`
    - `/powerauth-java-server/actuator/health/readiness`
...
to diagnose issues affecting PowerAuth Server’s availability and performance.

---

### Log Volume & Severity

In addition to raw logs in the central logging system, PowerAuth Server can expose **log event counters** as metrics. Monitoring the rate of log messages at different severity levels helps to detect problems early:

- A rise in **ERROR** logs can indicate internal failures or bugs in the server.
- A rise in **WARN** logs often indicates issues in **external systems** (downstream services, databases, message brokers) that are affecting PowerAuth Server, even if the server is still partially functioning.
