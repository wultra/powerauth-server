...
<!-- begin database table audit_log -->
### Audit Log Table

Stores audit records generated by the auditing library. Each record represents one logged event.

#### Columns

| Name              | Type         | Info                    | Note                                                                 |
|-------------------|--------------|-------------------------|----------------------------------------------------------------------|
| audit_log_id      | VARCHAR(36)  | primary key             | Unique identifier of the audit record (UUID).                        |
| application_name  | VARCHAR(256) | NOT NULL, index         | Name of the application that produced the audit log.                 |
| audit_level       | VARCHAR(32)  | NOT NULL, index         | Severity level of the audit record (e.g., INFO, WARN, ERROR).        |
| audit_type        | VARCHAR(256) | index                   | Type/category of the audit event.                                    |
| timestamp_created | TIMESTAMP    | DEFAULT NOW(), index    | Timestamp when the audit record was created.                         |
| message           | TEXT         | NOT NULL                | Human-readable audit message.                                        |
| exception_message | TEXT         | -                       | Exception message, if an error occurred.                             |
| stack_trace       | TEXT         | -                       | Stack trace associated with the exception.                           |
| param             | TEXT         | -                       | Serialized parameters related to the audit event.                    |
| calling_class     | VARCHAR(256) | NOT NULL                | Class that generated the audit record.                               |
| thread_name       | VARCHAR(256) | NOT NULL                | Name of the thread that created the audit record.                    |
| version           | VARCHAR(256) | -                       | Application version.                                                 |
| build_time        | TIMESTAMP    | -                       | Build timestamp of the application.                                  |
<!-- end -->

<!-- begin database table audit_param -->
### Audit Parameters Table

Stores key-value parameters associated with audit records.

#### Columns

| Name              | Type          | Info                 | Note                                                         |
|-------------------|---------------|----------------------|--------------------------------------------------------------|
| audit_log_id      | VARCHAR(36)   | index                | Reference to the related audit record.                       |
| timestamp_created | TIMESTAMP     | DEFAULT NOW(), index | Timestamp when the parameter was recorded.                   |
| param_key         | VARCHAR(256)  | index                | Parameter key.                                               |
| param_value       | VARCHAR(4000) | index                | Parameter value.                                             |
<!-- end -->

<!-- begin database table shedlock -->
### ShedLock Table

Stores distributed locks for scheduled tasks using the ShedLock library.

#### Columns

| Name       | Type         | Info        | Note                                                                 |
|------------|--------------|-------------|----------------------------------------------------------------------|
| name       | VARCHAR(64)  | primary key | Unique name of the scheduled lock.                                   |
| lock_until | TIMESTAMP    | NOT NULL    | Timestamp until which the lock is held.                              |
| locked_at  | TIMESTAMP    | NOT NULL    | Timestamp when the lock was acquired.                                |
| locked_by  | VARCHAR(255) | NOT NULL    | Identifier of the node that acquired the lock.                       |
<!-- end -->
