...
import com.wultra.security.powerauth.app.server.service.model.authentication.v3.OfflineSignatureRequest;
import com.wultra.security.powerauth.app.server.service.model.authentication.v3.*;
...
/**
 * Behavior class implementing the offline signature validation related processes. The class separates the
...
public class OfflineSignatureServiceBehavior {
    ...
    public CreateNonPersonalizedOfflineSignaturePayloadResponse createNonPersonalizedOfflineSignaturePayload(CreateNonPersonalizedOfflineSignaturePayloadRequest request) throws GenericServiceException {
        try {
            ...
            if (ecdsaSignatureBytes == null) {
                logger.error("Missing EC_P256 master key pair for application ID: {}", applicationId);
                // Rollback is not required, database is not used for writing
                throw localizationProvider.buildExceptionForCode(ServiceError.NO_MASTER_SERVER_KEYPAIR);
            }
            final String ecdsaSignature = Base64.getEncoder().encodeToString(ecdsaSignatureBytes);
