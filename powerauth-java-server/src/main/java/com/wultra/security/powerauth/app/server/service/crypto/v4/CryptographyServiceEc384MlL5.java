...
/**
 * Cryptography Service V4 implementation based on hybrid scheme with EC curve P-384 and ML-KEM-1026 + ML-DSA-87.
 *
...
@Slf4j
@AllArgsConstructor
public class CryptographyServiceEc384MlL5 extends CryptographyService {
    ...
    public KeyPair getMasterKeyPair(KeyType keyType, ApplicationEntity application) throws GenericServiceException {
        if (keyType != KeyType.ECDSA_P384 && keyType != KeyType.MLDSA_87) {
    ...
    public void storeDevicePublicKey(ActivationRecordEntity activation, BasePublicKey devicePublicKey) throws GenericServiceException {
        ...
        if (devicePublicKey instanceof EcPublicKey ecDevicePublicKey) {
            ...
            keyProvider.storeDevicePublicKey(activation, KeyType.MLDSA_87, pqcPublicKey);
        } else {
    ...
    public String generateActivationFingerprint(ActivationRecordEntity activation) throws GenericServiceException {
        try {
            ...
            final MLDSAPublicKey devicePublicKeyMldsa = (MLDSAPublicKey) devicePublicKeyRegistry.getPublicKey(KeyType.MLDSA_87).orElseThrow(() -> {
            ...
            final MLDSAPublicKey serverPublicKeyMldsa = (MLDSAPublicKey) serverPublicKeyRegistry.getPublicKey(KeyType.MLDSA_87).orElseThrow(() -> {
        ...
        return switch (keyType) {
            ...
            case MLDSA_87 -> {
                if (!algorithmQueryService.isAlgorithmSupported(application, SharedSecretAlgorithm.EC_P384_ML_L5)) {
                    logger.warn("Cryptography algorithm ML_L5 is not supported, application ID: {}", application.getId());
                    throw localizationProvider.buildExceptionForCode(ServiceError.CRYPTOGRAPHY_ALGORITHM_NOT_SUPPORTED);
                }
                try {
                    final KeyPair keyPair = getMasterKeyPair(keyType, application);
    ...
    public byte[] generateSignatureForActivation(KeyType keyType, byte[] data, ActivationRecordEntity activation) throws GenericServiceException {
        return switch (keyType) {
            ...
            case MLDSA_87 -> {
                try {
                    final PrivateKey serverPrivateKey = keyProvider.getServerPrivateKey(activation, KeyType.MLDSA_87).orElseThrow(() -> {
    ...
    public boolean verifySignatureForActivation(KeyType keyType, byte[] data, byte[] signature, ActivationRecordEntity activation) throws GenericServiceException {
        return switch (keyType) {
            ...
            case MLDSA_87 -> {
                try {
                    final PublicKey devicePublicKey = keyProvider.getDevicePublicKey(activation, KeyType.MLDSA_87).orElseThrow(() -> {
