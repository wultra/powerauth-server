 ...
 */
@Service("biometryServiceBehaviorV4")
@Slf4j
@AllArgsConstructor
public class BiometryServiceBehavior {
     ...
     * @throws GenericServiceException In case biometry setup fails.
     */
    @Transactional(rollbackFor = {RuntimeException.class, RollbackingServiceException.class})
    public AddBiometryResponse addBiometry(AddBiometryRequest request) throws GenericServiceException {
        try {
            ...
            final EncryptionContext context = new EncryptionContext(protocolVersion, applicationKey, activationId, EncryptorId.SETUP_BIOMETRY);
     ...
     * </ul>
     *
     * @throws GenericServiceException In case biometry remove fails.
     */
    @Transactional(rollbackFor = {RuntimeException.class, RollbackingServiceException.class})
    public void removeBiometry(RemoveBiometryRequest request) throws GenericServiceException {
        try {
            final String activationId = request.getActivationId();
            final ActivationRecordEntity activation = activationQueryService.findActivationForUpdate(activationId).orElseThrow(() -> {
                logger.info("Activation not found, activation ID: {}", activationId);
            ...
            activationValidator.validatePowerAuthProtocol(activation.getProtocol(), localizationProvider);
            activationValidator.validateActiveStatus(activation.getActivationStatus(), activation.getActivationId(), localizationProvider);
            removeBiometryFactorKey(activation);
        } catch (GenericServiceException ex) {
            // already logged
            throw ex;
        } catch (RuntimeException ex) {
    ...
    private void storeBiometryFactorKey(ActivationRecordEntity activation, SecretKey secretKey) throws GenericServiceException {
        ...
        activation.setBiometricFactorEnabled(true);
        if (activation.getBiometricFactorKey() == null) {
            activation.setBiometricFactorKey(secretKeyBase64);
        } else {
            activation.setBiometricFactorKeyNext(secretKeyBase64);
        }
        activationRepository.save(activation);
    }

    private void removeBiometryFactorKey(ActivationRecordEntity activation) throws GenericServiceException {
        activation.setBiometricFactorEnabled(false);
        activation.setBiometricFactorKey(null);
