...
import io.getlime.security.powerauth.app.server.converter.SignatureTypeVOConverter;
...
public class TokenBehavior {
    ...
    // Helper classes
    private final SignatureTypeVOConverter signatureTypeVOConverter = new SignatureTypeVOConverter();

    // Prepare logger
    private static final Logger logger = LoggerFactory.getLogger(TokenBehavior.class);

    @Autowired
    public TokenBehavior(RepositoryCatalogue repositoryCatalogue, LocalizationProvider localizationProvider, PowerAuthServiceConfiguration powerAuthServiceConfiguration, ServerPrivateKeyVOConverter serverPrivateKeyVOConverter) {
     ...
     */
    public com.wultra.security.powerauth.client.model.response.CreateTokenResponse createToken(com.wultra.security.powerauth.client.model.request.CreateTokenRequest request, KeyConvertor keyConversion) throws GenericServiceException {
        ...
        final com.wultra.security.powerauth.client.model.response.CreateTokenResponse response = new com.wultra.security.powerauth.client.model.response.CreateTokenResponse();
    ...
    private EciesCryptogram createToken(String activationId, String applicationKey, EciesCryptogram cryptogram, String signatureType, KeyConvertor keyConversion) throws GenericServiceException {
        try {
            ...
            final String serverPrivateKeyBase64 = serverPrivateKeyVOConverter.fromDBValue(serverPrivateKeyEncrypted, activation.getUserId(), activation.getActivationId());
            ...
            for (int i = 0; i < powerAuthServiceConfiguration.getGenerateTokenIdIterations(); i++) {
                final String tmpTokenId = tokenGenerator.generateTokenId();
    ...
    public ValidateTokenResponse validateToken(ValidateTokenRequest request) throws GenericServiceException {
        try {
            ...
            if (!tokenEntityOptional.isPresent()) {
                ...
                final com.wultra.security.powerauth.client.model.response.ValidateTokenResponse response = new com.wultra.security.powerauth.client.model.response.ValidateTokenResponse();
            ...
            if (isTokenValid) {
                ...
                response.setSignatureType(signatureTypeVOConverter.convertFrom(token.getSignatureTypeCreated()));
                return response;
            } else {
                final ValidateTokenResponse response = new ValidateTokenResponse();
                response.setTokenValid(false);
