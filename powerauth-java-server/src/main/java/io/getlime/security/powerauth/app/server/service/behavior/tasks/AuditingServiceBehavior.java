...
import io.getlime.security.powerauth.app.server.converter.JsonMappingVOConverter;
...
public class AuditingServiceBehavior {
    ...
    private final JsonMappingVOConverter jsonMappingVOConverter = new JsonMappingVOConverter();
     ...
     */
    public SignatureAuditResponse getSignatureAuditLog(String userId, Long applicationId, Instant startingDate, Instant endingDate) {

        final List<SignatureEntity> signatureAuditEntityList;
        ...
        if (signatureAuditEntityList != null) {
            for (SignatureEntity signatureEntity : signatureAuditEntityList) {
                ...
                item.setActivationStatus(activationStatusVOConverter.convert(signatureEntity.getActivationStatus()));
                item.setAdditionalInfo(jsonMappingVOConverter.mapFromString(signatureEntity.getAdditionalInfo()));
                ...
                item.setSignatureType(signatureTypeVOConverter.convertFrom(signatureEntity.getSignatureType()));
                item.setValid(signatureEntity.getValid());
                item.setVersion(signatureEntity.getVersion());
                item.setTimestampCreated(Instant.ofEpochMilli(signatureEntity.getTimestampCreated().getTime()));
    ...
    public void logSignatureAuditRecord(ActivationRecordEntity activation, SignatureData signatureData, SignatureType signatureType, Boolean valid, Integer version, String note, Date currentTimestamp) {
        ...
        signatureAuditRecord.setAdditionalInfo(jsonMappingVOConverter.writeToString(signatureData.getAdditionalInfo()));
