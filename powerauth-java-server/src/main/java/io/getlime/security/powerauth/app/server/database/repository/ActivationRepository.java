...
public interface ActivationRepository extends CrudRepository<ActivationRecordEntity, String> {
     ...
     */
    @Query("SELECT a FROM ActivationRecordEntity a WHERE a.userId IN ?1 AND (?2 IS NULL OR a.application.id IN ?2) AND (?3 IS NULL OR a.timestampLastUsed < ?3) AND (?4 IS NULL OR a.timestampLastUsed >= ?4) AND a.activationStatus IN ?5")
