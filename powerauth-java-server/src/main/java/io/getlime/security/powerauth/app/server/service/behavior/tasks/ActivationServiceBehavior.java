...
import io.getlime.security.powerauth.app.server.converter.ActivationOtpValidationVOConverter;
import io.getlime.security.powerauth.app.server.converter.ActivationStatusVOConverter;
import io.getlime.security.powerauth.app.server.converter.RecoveryPukVOConverter;
...
public class ActivationServiceBehavior {
    ...
    private final ActivationOtpValidationVOConverter activationOtpValidationConverter = new ActivationOtpValidationVOConverter();
    private ServerPrivateKeyVOConverter serverPrivateKeyVOConverter;
    private RecoveryPukVOConverter recoveryPukVOConverter;
    ...
    @Autowired
    public void setRecoveryPukConverter(RecoveryPukVOConverter recoveryPukVOConverter) {
        this.recoveryPukVOConverter = recoveryPukVOConverter;
    }
     ...
     */
    public GetActivationListForUserResponse getActivationList(Long applicationId, String userId) {
        ...
        if (activationsList != null) {
            for (ActivationRecordEntity activation : activationsList) {
                ...
                activationServiceItem.setActivationStatus(activationStatusVOConverter.convert(activation.getActivationStatus()));
                ...
                activationServiceItem.setTimestampCreated(Instant.ofEpochMilli(activation.getTimestampCreated().getTime()));
                activationServiceItem.setTimestampLastUsed(Instant.ofEpochMilli(activation.getTimestampLastUsed().getTime()));
                activationServiceItem.setTimestampLastChange(Instant.ofEpochMilli(activation.getTimestampLastChange().getTime()));
     ...
     * @return Response with list of matching activations.
     */
    public LookupActivationsResponse lookupActivations(List<String> userIds, List<Long> applicationIds, Instant timestampLastUsedBefore, Instant timestampLastUsedAfter, ActivationStatus activationStatus, List<String> activationFlags) {
        ...
        try {
            ...
            if (activation != null) {
                ...
                if (activation.getActivationStatus() == io.getlime.security.powerauth.app.server.database.model.ActivationStatus.CREATED) {
                    ...
                    final String randomStatusBlobNonce = challenge == null ? null : BaseEncoding.base64().encode(keyGenerator.generateRandomBytes(16));

                    // Activation signature
                    final MasterKeyPairEntity masterKeyPairEntity = masterKeyPairRepository.findFirstByApplicationIdOrderByTimestampCreatedDesc(activation.getApplication().getId());
                    ...
                    final byte[] masterPrivateKeyBytes = BaseEncoding.base64().decode(masterPrivateKeyBase64);
                    ...
                    response.setTimestampCreated(Instant.ofEpochMilli(activation.getTimestampCreated().getTime()));
                    response.setTimestampLastUsed(Instant.ofEpochMilli(activation.getTimestampLastUsed().getTime()));
                    response.setTimestampLastChange(Instant.ofEpochMilli(activation.getTimestampLastChange().getTime()));
                    ...
                    if (devicePublicKeyBase64 != null) {
                        ...
                        final PrivateKey serverPrivateKey = keyConversionUtilities.convertBytesToPrivateKey(BaseEncoding.base64().decode(serverPrivateKeyBase64));
                        final PublicKey devicePublicKey = keyConversionUtilities.convertBytesToPublicKey(BaseEncoding.base64().decode(devicePublicKeyBase64));
                        final PublicKey serverPublicKey = keyConversionUtilities.convertBytesToPublicKey(BaseEncoding.base64().decode(serverPublicKeyBase64));
                        ...
                        final byte[] ctrDataHashForStatusBlob;
                ...
                final GetActivationStatusResponse response = new GetActivationStatusResponse();
                response.setActivationId(activationId);
                response.setActivationStatus(activationStatusVOConverter.convert(ActivationStatus.REMOVED));
                ...
                response.setTimestampCreated(Instant.ofEpochMilli(0));
                response.setTimestampLastUsed(Instant.ofEpochMilli(0));
     ...
     */
    public InitActivationResponse initActivation(Long applicationId, String userId, Long maxFailureCount, Instant activationExpireTimestamp,
        ...
        try {
            ...
            Instant timestampExpiration = activationExpireTimestamp;
            if (timestampExpiration == null) {
                timestampExpiration = Instant.ofEpochMilli(timestamp.getTime() + powerAuthServiceConfiguration.getActivationValidityBeforeActive());
            }
            ...
            final byte[] masterPrivateKeyBytes = BaseEncoding.base64().decode(masterKeyPair.getMasterKeyPrivateBase64());
            ...
            final String activationSignatureBase64 = BaseEncoding.base64().encode(activationSignature);
            ...
            activation.setTimestampActivationExpire(new Date(timestampExpiration.toEpochMilli()));
            ...
            final ServerPrivateKey serverPrivateKey = serverPrivateKeyVOConverter.toDBValue(serverKeyPrivateBytes, userId, activationId);
    ...
    public PrepareActivationResponse prepareActivation(String activationCode, String applicationKey, EciesCryptogram eciesCryptogram, KeyConvertor keyConversion) throws GenericServiceException {
        try {
            ...
            final byte[] devicePublicKeyBytes = BaseEncoding.base64().decode(request.getDevicePublicKey());
            PublicKey devicePublicKey = null; //TODO:Check if remains null
            ...
            final String ctrDataBase64 = BaseEncoding.base64().encode(ctrData);
            ...
            final String encryptedData = BaseEncoding.base64().encode(responseCryptogram.getEncryptedData());
            final String mac = BaseEncoding.base64().encode(responseCryptogram.getMac());
            ...
            encryptedResponse.setActivationStatus(activationStatusVOConverter.convert(activationStatus));
    ...
    public CreateActivationResponse createActivation(
            String userId,
            Instant activationExpireTimestamp,
        ...
        try {
            ...
            final InitActivationResponse initResponse = this.initActivation(application.getId(), userId, maxFailureCount, activationExpireTimestamp, activationOtpValidation, activationOtp, keyConversion);
            ...
            encryptedResponse.setEncryptedData(encryptedData);
            encryptedResponse.setMac(mac);
            encryptedResponse.setActivationStatus(activationStatusVOConverter.convert(activation.getActivationStatus()));
            return encryptedResponse;
        } catch (InvalidKeySpecException ex) {
            logger.error(ex.getMessage(), ex);
    ...
    public RecoveryCodeActivationResponse createActivationUsingRecoveryCode(RecoveryCodeActivationRequest request, KeyConvertor keyConversion) throws GenericServiceException {
        try {
            ...
            final byte[] nonceBytes = request.getNonce() != null ? BaseEncoding.base64().decode(request.getNonce()) : null;
            ...
            final RecoveryCodeEntity recoveryCodeEntity = recoveryCodeRepository.findByApplicationIdAndRecoveryCode(application.getId(), recoveryCode);
            ...
            for (RecoveryPukEntity recoveryPukEntity: recoveryCodeEntity.getRecoveryPuks()) {
                if (RecoveryPukStatus.VALID.equals(recoveryPukEntity.getStatus())) {
                    if (firstValidPuk == null) {
                        ...
                        final String pukHash = recoveryPukVOConverter.fromDBValue(recoveryPuk, application.getId(), recoveryCodeEntity.getUserId(), recoveryCode, recoveryPukEntity.getPukIndex());
                        try {
            ...
            // Extract the device public key from request
            final byte[] devicePublicKeyBytes = BaseEncoding.base64().decode(layer2Request.getDevicePublicKey());
            final PublicKey devicePublicKey;
            ...
            final ActivationRecovery activationRecovery = createRecoveryCodeForActivation(activation, false);
    ...
    private ActivationRecovery createRecoveryCodeForActivation(ActivationRecordEntity activationEntity, boolean isActive) throws GenericServiceException {
        ...
        try {
            ...
            for (int i = 0; i < powerAuthServiceConfiguration.getGenerateRecoveryCodeIterations(); i++) {
                ...
                final boolean recoveryCodeExists = recoveryCodeRepository.getRecoveryCodeCount(applicationId, recoveryInfo.getRecoveryCode()) > 0;
            ...
            final RecoveryPuk recoveryPuk = recoveryPukVOConverter.toDBValue(pukHash, applicationId, userId, recoveryCode, recoveryPukEntity.getPukIndex());
