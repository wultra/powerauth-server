...
import io.getlime.security.powerauth.app.server.converter.RecoveryCodeStatusVOConverter;
import io.getlime.security.powerauth.app.server.converter.RecoveryPukStatusConverter;
...
public class RecoveryServiceBehavior {
    ...
    private final RecoveryPrivateKeyVOConverter recoveryPrivateKeyVOConverter;
    ...
    private final RecoveryCodeStatusVOConverter recoveryCodeStatusVOConverter = new RecoveryCodeStatusVOConverter();
    private final RecoveryPukStatusConverter recoveryPukStatusConverter = new RecoveryPukStatusConverter();
    private final RecoveryPukVOConverter recoveryPukVOConverter;
    ...
    public RecoveryServiceBehavior(LocalizationProvider localizationProvider,
                                   ...
                                   ServerPrivateKeyVOConverter serverPrivateKeyVOConverter, RecoveryPrivateKeyVOConverter recoveryPrivateKeyVOConverter,
                                   RecoveryPukVOConverter recoveryPukVOConverter) {
        this.localizationProvider = localizationProvider;
        this.powerAuthServiceConfiguration = powerAuthServiceConfiguration;
        this.repositoryCatalogue = repositoryCatalogue;
        this.serverPrivateKeyVOConverter = serverPrivateKeyVOConverter;
        this.recoveryPrivateKeyVOConverter = recoveryPrivateKeyVOConverter;
    ...
    public CreateRecoveryCodeResponse createRecoveryCode(CreateRecoveryCodeRequest request, KeyConvertor keyConversion) throws GenericServiceException {
        try {
            ...
            if ((recoveryConfigEntity.getActivationRecoveryEnabled() == null) || !recoveryConfigEntity.getAllowMultipleRecoveryCodes()) {
                final List<RecoveryCodeEntity> existingRecoveryCodes = recoveryCodeRepository.findAllByApplicationIdAndUserId(applicationId, userId);
            ...
            final EncryptionMode encryptionMode = recoveryConfigEntity.getPrivateKeyEncryption();
            final String recoveryPrivateKeyBase64 = recoveryConfigEntity.getRecoveryPostcardPrivateKeyBase64();
            final RecoveryPrivateKey recoveryPrivateKeyEncrypted = new RecoveryPrivateKey(encryptionMode, recoveryPrivateKeyBase64);
            final String decryptedPrivateKey = recoveryPrivateKeyVOConverter.fromDBValue(recoveryPrivateKeyEncrypted, applicationId);
            ...
            for (int i = 0; i < powerAuthServiceConfiguration.getGenerateRecoveryCodeIterations(); i++) {
                final RecoveryInfo recoveryInfo = identifierGenerator.generateRecoveryCode(secretKey, (int) pukCount, true);
            ...
            final CreateRecoveryCodeResponse response = new CreateRecoveryCodeResponse();
            ...
            for (int i = 1; i <= pukDerivationIndexes.size(); i++) {
                final Long pukDerivationIndex = pukDerivationIndexes.get(i);
                final RecoveryCodePuk pukResponse = new RecoveryCodePuk();
                ...
                response.getPuks().add(pukResponse);
            }
            return response;
    ...
    public LookupRecoveryCodesResponse lookupRecoveryCodes(LookupRecoveryCodesRequest request) throws GenericServiceException {
        ...
        final String activationId = request.getActivationId();
        final String userId = request.getUserId();
        final RecoveryCodeStatus recoveryCodeStatus = recoveryCodeStatusVOConverter.convertTo(request.getRecoveryCodeStatus());
        ...
        final LookupRecoveryCodesResponse response = new LookupRecoveryCodesResponse();
        for (RecoveryCodeEntity recoveryCodeEntity: recoveryCodesEntities) {
            ...
            recoveryCode.setStatus(recoveryCodeStatusVOConverter.convertFrom(recoveryCodeEntity.getStatus()));
            for (RecoveryPukEntity recoveryPukEntity: recoveryCodeEntity.getRecoveryPuks()) {
                final RecoveryCodePuk puk = new RecoveryCodePuk();
            ...
            response.getRecoveryCodes().add(recoveryCode);
        }
        return response;
    }
    ...
    public RevokeRecoveryCodesResponse revokeRecoveryCodes(RevokeRecoveryCodesRequest request) throws GenericServiceException {
        ...
        int revokedCount = 0;
        for (Long recoveryCodeId : recoveryCodeIds) {
            final Optional<RecoveryCodeEntity> recoveryCodeOptional = recoveryCodeRepository.findById(recoveryCodeId);
            if (!recoveryCodeOptional.isPresent()) {
                // Silently ignore invalid recovery code IDs
                continue;
            }
            final RecoveryCodeEntity recoveryCode = recoveryCodeOptional.get();
    ...
    public UpdateRecoveryConfigResponse updateRecoveryConfig(UpdateRecoveryConfigRequest request, KeyConvertor keyConversion) throws GenericServiceException {
        try {
            ...
            if (request.isRecoveryPostcardEnabled() && recoveryConfigEntity.getRecoveryPostcardPrivateKeyBase64() == null) {
                ...
                final byte[] privateKeyBytes = keyConversion.convertPrivateKeyToBytes(privateKey);
                final byte[] publicKeyBytes = keyConversion.convertPublicKeyToBytes(publicKey);
                final String publicKeyBase64 = BaseEncoding.base64().encode(publicKeyBytes);

                final RecoveryPrivateKey recoveryPrivateKey = recoveryPrivateKeyVOConverter.toDBValue(privateKeyBytes, applicationId);
            ...
            recoveryConfigRepository.save(recoveryConfigEntity);
            final UpdateRecoveryConfigResponse response = new UpdateRecoveryConfigResponse();
