...
/**
 * Model class representing request for protocol upgrade start (V4).
 *
...
public class StartUpgradeRequest {
    ...
    @NotBlank(message = "Temporary key ID must not be empty when starting upgrade")
    ...
    @NotBlank(message = "Nonce must not be empty when starting upgrade")
    @ToString.Exclude
    ...
    @NotNull(message = "Timestamp must not be null when starting upgrade")
    @Positive(message = "Timestamp must be positive when starting upgrade")
